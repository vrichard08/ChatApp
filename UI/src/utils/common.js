...
export function formatNumber(numStr, separator = ' ', decimal = '.') {
  numStr = numStr + '';
  if (numStr.includes(decimal)) {
    let [base, dec] = numStr.split(decimal);
    return `${formatNumberWithoutDecimal(base, separator)}${decimal}${dec}`;
  } else {
    return formatNumberWithoutDecimal(numStr, separator);
  }
}
export function formatNumberWithoutDecimal(numStr, separator = ' ') {
...
export function formatNumberWithDecimal(
  ...
  numStr = numStr.toFixed(decimalNumber ? decimalNumber : 0) + '';
  if (numStr.includes(decimal)) {
    let [base, dec] = numStr.split(decimal);
    return `${formatNumberWithoutDecimal(base, separator)}${decimal}${dec}`;
  } else {
...
export function hidrateForm(form, entity) {
  if (!form || !entity) {
    return;
  }
...
export const vttToText = (vtt) => {
  ...
  let appendToTempLine = (line) => {
    if (tempLine && !tempLine.endsWith(' ')) {
      tempLine += ' ';
    }
    tempLine += line;
  };
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      /(\d\d:)?\d\d:\d\d\.\d\d\d --> (\d\d:)?\d\d:\d\d\.\d\d\d/gi.test(line)
    ) {
      continue;
    }

    if (isEndOfLine(line)) {
      appendToTempLine(line);
